...
resource "aws_route_table" "private" {
  ...
  tags = merge(
    {
      "Name" = var.single_nat_gateway ? "${var.name}-${var.private_subnet_suffix}-${element(var.azs, count.index)}" : format(
...
resource "aws_subnet" "public" {
  ...
  tags = merge(
    {
      "Name" = format(
       var.public_subnet_name_tags[count.index]
      )
    },
    var.tags,
...
resource "aws_subnet" "private" {
  ...
  tags = merge(
    {
      "Name" = format(
        var.private_subnet_name_tags[count.index]
      )
  },
  var.tags,
